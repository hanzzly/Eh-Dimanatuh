...
class Main {
  ...
  async getInput() {
    this.setting.baseUrl = "https://edwebservices.engdis.com/api/";
    this.setting.username = await prompt("[?] enter your username : ");
    this.setting.password = await prompt("[?] enter your password : ");
    console.log();
  }
      ...
      for (const item of courseTree.data) {
        ...
        for (const elem of item.Children) {
          if (elem.Name != "Test") {
            ...
            // Mark the Step itself as viewed
            await this.engdis.setSucessTask(course.ParentNodeId, elem.NodeId);
          } else {
            console.log(`[#] checking ${elem.Name}`)
            await this.setTest100Percent(item["Metadata"]["Code"], item["NodeId"], item["ParentNodeId"])
          }
        }
        // Mark the Lesson itself as viewed
        await this.engdis.setSucessTask(course.ParentNodeId, item.NodeId);
      }
